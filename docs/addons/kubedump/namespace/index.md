...
```bash
❯ kubectl apply -f ./docs/addons/kubedump/namespace/examples/repository.yaml
repository.stash.appscode.com/gcs-repo created
```

```bash
❯ kubectl apply -f ./docs/addons/kubedump/namespace/examples/rbac.yaml
...
```bash
❯ kubectl apply -f ./docs/addons/kubedump/namespace/examples/backupconfiguration.yaml
backupconfiguration.stash.appscode.com/namespace-kubedump created
```

```bash
❯ kubectl get backupconfiguration -n demo
NAME                        TASK                    SCHEDULE      PAUSED   PHASE   AGE
namespace-kubedump   kubedump-backup-0.1.0   */5 * * * *            Ready   85m
```

```bash
❯ kubectl get backupsession -n demo -w
NAME                                   INVOKER-TYPE          INVOKER-NAME                PHASE   DURATION   AGE
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump                      0s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Pending              0s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              0s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              11s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              32s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              45s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              45s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              45s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Running              45s
namespace-kubedump-1651060201   BackupConfiguration   namespace-kubedump   Succeeded   46s        45s
...
gcs-repo   true        610.412 KiB   5                116s                     86m
```

```bash
❯ kubectl stash download -n demo gcs-repo  --destination=$HOME/Downloads/stash --snapshots="latest"
I0427 17:52:30.138254 1095200 download.go:176] Running docker with args: [run --rm -u 1000 -v /tmp/scratch:/tmp/scratch -v /home/emruz/Downloads/stash:/tmp/destination --env HTTP_PROXY= --env HTTPS_PROXY= --env-file /tmp/scratch/config/restic-envs stashed/restic:latest --no-cache restore latest --target /tmp/destination/latest]
I0427 17:53:03.776042 1095200 download.go:181] Output: restoring <Snapshot f5f2cb4d of [/tmp/manifests] at 2022-04-27 11:50:12.278580793 +0000 UTC by @host-0> to /tmp/destination/latest

I0427 17:53:03.777863 1095200 download.go:137] Snapshots: [latest] of Repository demo/gcs-repo restored in path /home/emruz/Downloads/stash
❯ cd $HOME/Downloads/stash
❯ ls
latest
❯ cd latest/tmp/manifests/
❯ ls
ConfigMap           DaemonSet   Endpoints      Event  Pod         Role         Secret   ServiceAccount
ControllerRevision  Deployment  EndpointSlice  Lease  ReplicaSet  RoleBinding  Service
❯ ls Deployment
coredns.yaml  stash-stash-enterprise.yaml
❯ cat Deployment/coredns.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  annotations:
